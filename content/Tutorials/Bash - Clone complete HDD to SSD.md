...
4. Backup the host HDD using the `dd` command. Firstly, **we should check which disk and partitions hold the Linux system**. This is especially so if we have more than one disk on the machine. For that purpose, we can use `cfdisk` command. Follow the [[Bash - Clone Raspberry Pi OS to an SD Card|blog post]] to create image of your entire HDD to avoid data loss.![[clone_HDD_03.png|sda3 is my boot drive. The interface may be different according to your Linux distro.]]
